...
def parse_valid(msg_name, msg_type, msg_len, if_itch_f, assign_f, tb_port_f, tb_sig_f, if_early_f, early_assign_f,port_f, early_port_f, assign_mux_f , dec_port_f):
    ...
    #mux
    dec_sig_name = "dec_"+msg_name+"_v_i"
    assign_mux_f.write("\nassign "+sig_name+" = |"+dec_sig_name+";\n")    

    #top
    dec_port_f.write("\n."+sig_name+"("+"dec_"+msg_name+"_v[i]),\n")
...
def parse_field(msg_name, field, if_itch_f, assign_f, tb_port_f, tb_sig_f, if_early_f, early_assign_f, port_f, early_port_f, assign_mux_f, dec_port_f ):
    f_name = field['@name']
    f_len  = field['@len']
    ...
    if not( f_name == "message_type" ):
        ...
        #mux
        dec_v_sig_name = "dec_"+msg_name+"_v_i"
        dec_sig_name = "dec_"+msg_name+"_"+f_name+"_i"
        len_str = f_len+"*LEN"
        assign_mux_f.write("assign "+sig_name+" = ")
        for i in range(2): 
            if not( i == 0 ):
                assign_mux_f.write("\n|") 
            assign_mux_f.write("{"+len_str+"{"+dec_v_sig_name+"["+str(i)+"]}} & "+ dec_sig_name+"["+str(i)+"*"+len_str+"+"+len_str+"-1:"+str(i)+"*"+len_str+"]")    
        assign_mux_f.write(";\n")       
       
        # top
        dec_port_f.write("."+sig_name+"("+"dec_"+msg_name+"_"+f_name +"[i*"+len_str+"+"+len_str+"-1:i*"+len_str+"]),\n")
...
def main():
    ...
    dec_port_f = open(DEC_PORT_FILE,"w")
    ...
    assign_mux_f = open(ASSIGN_MUX_LOGIC_FILE,"w")
    ...
    for struct in structs:
        ...
        msg_type=struct['@id']
        msg_len=struct['@len']
        sig_v = parse_valid(msg_name , msg_type, msg_len, if_itch_f, assign_f, tb_port_f, tb_sig_f, if_early_f , early_assign_f, port_f, early_port_f, assign_mux_f, dec_port_f)
        ...
        for field in struct['Field']:
            ...
            if type(field) is dict:
                sig_field = parse_field(msg_name, field, if_itch_f, assign_f, tb_port_f, tb_sig_f,if_early_f , early_assign_f, port_f, early_port_f, assign_mux_f, dec_port_f)
                if len(sig_field) > 0:
